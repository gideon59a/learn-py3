# Ref: https://www.youtube.com/watch?v=q7HVghYjwYo
# (which is the continuation of https://www.youtube.com/watch?v=2dEM-s3mRLE)
...
my_logger = Alogger('test-srv.log')
...
login_manager.login_view = 'login'  # When accessing a route that requires login, and the login_required fails, then
  # instead of getting an "Unauthorized" message, this configureation will redirect to the login path.
...
def logmein():
    ...
    db_user = User.query.filter_by(username=in_username).first()
    if not db_user:
        return '<h1>User not found!</h1>'
    login_user(db_user)
    return f'<h1>User {in_username} found!</h1>'

@app.route('/logmein-py', methods=['POST'])
def logmein_py():
    ''' Python client login '''
    request_dict = json.loads(request.get_data().decode("utf-8"))
    in_username = request_dict['username']
    db_user = User.query.filter_by(username=in_username).first()
    if not db_user:
        return '<h1>User not found!</h1>'
    login_user(db_user)
    return jsonify({"Status": "ok", "logged_in": in_username})
...
@login_required
def home():
    return jsonify({"current user": current_user.username})
